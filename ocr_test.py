...
def detect_license_plate(img_path: str):
    ...
    img_path: 이미지 파일 경로
    """
    img = cv2.imread(str(img_path))  # 이미지 불러오기
    if img is None:  # 이미지 불러오기 실패 시 에러 메시지 반환
    ...
    for contour in contours:
        ...
        if len(approx) == 4:  # approx 리스트에 포함된 점의 수가 4개인지 확인
            # print('type(contour)', type(contour),'contour.shape ', contour.shape)
            # print('type(approx)', type(approx), 'approx.shape', approx.shape)
            # contour에서 사각형 정보 얻기
